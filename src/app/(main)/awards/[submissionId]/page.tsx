...
export const generateMetadata: GenerateMetadata<RouteParams> = async ({ params }) => {
  ...
  const imageSrc = 'https://www.qceventplanning.com/images/award-of-excellence.png';
  ...
  return {
    ...
    openGraph: {
      url: `https://www.qceventplanning.com/awards/${award.submissionId}`,
      type: 'website',
      title,
      description,
...
const AwardPage: PageComponent<RouteParams> = async ({ params }) => {
  ...
  const url = `https://www.qceventplanning.com/awards/${award.submissionId}`;
  const suggestedText = `I just earned this Award of Excellence from ${award.schoolName} for my work in ${award.courseName}! I'm so excited to be pursuing my dream career. 💫 #AwardOfExcellence @QCEventPlanning`;
